...
def pytest_collection_modifyitems(config, items):
    """Skip this directory when the gitignored corpus binaries are absent.

    The corpus is the ONLY gate left. Extraction used to also require a live
    local-model runtime; the engine is now a committed drafts recording replayed
    in pure Python, so nothing here needs a GPU, an API key, or a network.
    """
